...
class PasswordChangeController @Inject() (
  ...
  def handleStartResetPassword = Action.async { implicit request =>
    pwResetForm.bindFromRequest.fold (
      errors => {
        Future.successful(BadRequest(views.html.account.startResetPassword(errors)))
      },
      email => {

        authInfoRepository.find(LoginInfo(CredentialsProvider.ID,email))(ClassTag(classOf[PasswordInfo])).flatMap {
          case Some(user) => {
          ...
          case None => {
            Future.successful(Ok(views.html.account.unknownEmail()))
          }
        }
      }
    )
  ...
  def specifyResetPassword (tokenId: String) = Action.async { implicit request =>
    tokenService.retrieve(tokenId).flatMap {
      case Some(token) if (!token.isSignUp && !token.isExpired) => {
        Future.successful(Ok(views.html.account.specifyResetPassword(tokenId, passwordsForm)))
      }
      case Some(token) => {
  ...
  def handleResetPassword = Action.async { implicit request =>
    passwordsForm.bindFromRequest.fold(
      formWithErrors => Future.successful(BadRequest(views.html.account.specifyResetPassword(formWithErrors.data("token"), formWithErrors))),
      passwords => {
            ...
            userService.retrieve(loginInfo).flatMap {
              case Some(user) => {
                ...
                env.authenticatorService.create(user.loginInfo).flatMap { authenticator =>
                  ...
                  Future.successful(Ok(views.html.account.confirmResetPassword(user)))
                }
              }
  ...
  def startChangePassword = SecuredAction.async { implicit request =>
    Future.successful(Ok(views.html.account.changePassword(request.identity,changePasswordForm)))
  }
  ...
  def handleChangePassword = SecuredAction.async { implicit request =>
    changePasswordForm.bindFromRequest.fold(
      formWithErrors => Future.successful(BadRequest(views.html.account.changePassword(request.identity,formWithErrors))),
      changeInfo => {
        ...
        credentialsProvider.authenticate( Credentials(user.email.getOrElse(""), changeInfo.currentPassword) ).flatMap { loginInfo =>
          ...
          case e: ProviderException =>
            BadRequest(views.html.account.changePassword(request.identity,changePasswordForm.withError("currentPassword","Does not match current password!")))
        }
      }
    )
  }
